...
SELF.initialize = function(game_id, index, callback){
...
SELF.update_buffs = function(game_id, index, user_id, prebuffs, callback){
	PRM.cols.games.findById(game_id, function(err, game){
		var unit_constants = {
			"hero": game.user_data[0].stats,
			"enemy": game.user_data[1].stats
		...
		function apply_buffs(){
			...
			if (user_id == game.user_data[0]._id){var unit_alias = "hero"}
			else if (user_id == game.user_data[1]._id){var unit_alias = "enemy"}

			if (prebuffs.length > 0){
				if (next_turn.unit_stats[unit_alias].buffs.length < 10){
					
					for (var i = 0; i < prebuffs.length; ++i) {
						var p = prebuffs[i];
						turn.sequence[turn.index + 1].unit_stats[unit_alias].buffs.push(p);
						
						prebuffs.splice(i,1); i--;
					};

					var recalc_turns = false;
					var recalc_duration = 0;

					for (var i = 0; i < next_turn.unit_stats[unit_alias].buffs.length; ++i) {
						var p = next_turn.unit_stats[unit_alias].buffs[i];
		...
		function calculate_turns(index, splice) {
			...
			while (temp_sequence.length < turn.sequence_max) {
				...
				var target_random = Math.floor(Math.random() * target_dice.length);
				var target = target_dice[target_random];
				if (splice == undefined){var unit_stats_temp = PRM.clone_obj(unit_stats)} else {var unit_stats_temp = PRM.clone_obj(turn.sequence[index].unit_stats)};
